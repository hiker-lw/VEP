...
# NOTE: This is the third path-patching experiment. At a specific token position, the corresponding Xc is designed,
# and path patching is then used to perturb each head before layer 30. The logit-ratio change of the correct answer
# on the residual stream at layer 30 is computed before and after perturbation, considering only the logits of "yes" and "no".
# The goal is to identify the head that causes the largest drop in this ratio.
...
def add_pre_hook(module, name, cache, pos=None, read=False):
    """Add a hook to the module to save or replace the module input.

    Parameters
    ----------
    module : nn.Module
        The module whose input needs to be replaced or saved.
    name : str
        The name used when saving the input in the cache.
    cache : Dict
        The cache used to store the input.
    pos : int, optional
        The position of the token in the sentence whose value needs to be saved or replaced, by default None.
    read : bool, optional
        Whether to save or replace the input. If True, the input is saved, by default False.
    """
...
def add_pre_hook_single_head(module, name, cache, pos, read=False, head_idx=None):
    """The parameters are the same as in the previous function, except that this function can replace only the output corresponding to a single attention head."""
...
def path_patching(args_Xr, args_Xc, tokenizer, model, image_processor, llava_model_name, probing_word="?"):
    ...
    Xc_word = "bomb"
    Xc_word_token = tokenizer.encode(Xc_word)[1]

    correct_answer = "Yes"
    wrong_answer = "No"
    correct_answer_token = tokenizer.encode(correct_answer)[1]
    wrong_answer_token = tokenizer.encode(wrong_answer)[1]
    
    # Initialize a matrix to record the results
    results = torch.zeros(size=(num_layers, num_attention_heads))

    # Caches for the internal model representations corresponding to Xr and Xc
    Hr = {}
    Hc = {}
    ...
    yes_logits = logits[0, probing_token_pos, correct_answer_token].item()
    no_logits = logits[0, probing_token_pos, wrong_answer_token].item()
    softmax_probs = torch.nn.functional.softmax(torch.tensor([yes_logits, no_logits]), dim=-1)
    prob = softmax_probs[0].item()
    ...
    probing_token_pos_Xc = input_ids[0].tolist().index(Xc_word_token) + 24 * 24 - 1
    hooks = []
    for i in range(num_layers):
    ...
    for source_layer in tqdm.tqdm(range(0, key_layer)):
        for source_head_idx in list(range(num_attention_heads)):
            # Forward B: during the forward pass, keep the other attention heads fixed and replace only the target attention head
            hooks = []
...
Xc_prompt = "Is there a bomb in the image?"
...
if __name__ == "__main__":
    ...
    # Get the number of model layers, the number of attention heads, dimensions, etc.
    ...
    for question_id in tqdm.tqdm(random.sample(analysis_question_id_list, 5)):
        ...
        with open("./path_patching/object_token_encode_<Yes>_logits_Xr_Xc_third_path_patching_results_pope_non_hallucination_random_5_samples_label_yes_temp_0_key_layer_12.pkl", "wb") as f:
            pickle.dump(results_dict, f)
    
    with open("./path_patching/object_token_encode_<Yes>_logits_Xr_Xc_third_path_patching_results_pope_non_hallucination_random_5_samples_label_yes_temp_0_key_layer_12.pkl", "rb") as f:
    ...
    show_pp(average_results, save_fig_path="./path_patching/object_token_encode_<Yes>_logits_Xr_Xc_third_path_patching_results_pope_non_hallucination_random_5_samples_label_yes_temp_0_key_layer_12.png")